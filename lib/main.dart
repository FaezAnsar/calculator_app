  ...
  try {
    ...
    try {
      if (duplicateTest.contains("%")) {
        // Split by operators
        List<String> arr =
            duplicateTest.split(RegExp(r'(?=[+\-x÷%])|(?<=[+\-x÷%])'));
        List<String> modifiedArr = [];
        int i = 0;
        ...
        while (i < arr.length) {
          ...
          if (arr[i] == "%") {
            ...
            // Insert parentheses before the number if it exists
            if (i > 0 && arr[i - 1].contains(RegExp(r'\d'))) {
              modifiedArr.insert(i - 1, "(");
            }

            // Insert closing parenthesis after, but ensure any operator following "%" is properly added back
            if (i + 1 < arr.length && arr[i + 1].contains(RegExp(r'[+\-x÷]'))) {
              modifiedArr.add(")");
              modifiedArr.add(arr[
                  i + 1]); // Re-insert operator after the closing parenthesis
              i += 2; // Skip to next after modifications
            } else {
              modifiedArr.add(")"); // Insert closing parenthesis
              i++;
            }
          } else {
            i++;
      ...
      // Replace custom operators with valid math operators
      duplicateTest = duplicateTest.replaceAll('x', '*').replaceAll('÷', "/");
      //print("duplicate: $duplicateTest");
    } catch (e) {
      log("Error during percentage handling: $e");
    }

    // Check the last character and remove it if it's an operator
    String lastElement = duplicateTest[duplicateTest.length - 1];
    try {
      if (lastElement != ")") {
        int.parse(lastElement); // Check if it's a number
      }
    } catch (e) {
      // Remove the last character if it's not a valid number or closing parenthesis
      duplicateTest = duplicateTest.substring(0, duplicateTest.length - 1);
    }
                        ...
                        children: [
                          FloatingActionButton(
                            ...
                            child: Text(
                              "AC",
                              style: TextStyle(
                                  color: Color(0xff2E2F30), fontSize: 25),
                            ),
                          ...
                          FloatingActionButton(
                            onPressed: () {
                              setState(() {
                                if (typingText.isNotEmpty &&
                                    typingText[typingText.length - 1] != "%") {
                            ...
                            child: Text(
                              "%",
                              style: TextStyle(
                                  color: Colors.yellow.shade600, fontSize: 19),
                            ),
                            backgroundColor: Color(0xff2E2F30),
                          ),
                          ...
                          FloatingActionButton(
                            onPressed: () {
                              setState(() {
                                if (typingText.isEmpty ||
                                    typingText[typingText.length - 1] == "÷") {
                                } else if (typingText[typingText.length - 1] ==
                                        "x" ||
                                    typingText[typingText.length - 1] == "-" ||
                                    typingText[typingText.length - 1] == "+") {
                                  typingText =
                                      "${typingText.substring(0, typingText.length - 1)}";
                                  if (typingText.isEmpty ||
                                      typingText[typingText.length - 1] ==
                                          "÷") {
                                  } else {
                            ...
                            child: Text(
                              "÷",
                              style: TextStyle(
                                  color: Colors.yellow.shade600, fontSize: 25),
                            ),
                            backgroundColor: Color(0xff2E2F30),
                          ),
                          ...
                          FloatingActionButton(
                            onPressed: () {
                              setState(() {
                                if (typingText != "") {
                                  typingText = typingText.substring(
                                      0, typingText.length - 1);
                                  equalsPressed = false;
                                  if (typingText.isNotEmpty &&
                                      typingText[typingText.length - 1] !=
                                          "(") {
                            ...
                            backgroundColor: Color(0xff2E2F30),
                          ) //style: ButtonStyle(backgroundColor:MaterialStatePropertyAll(Colors.yellow.shade600),shape: MaterialStatePropertyAll(CircleBorder()),padding: MaterialStatePropertyAll(EdgeInsets.all(20))),),
                        ],
                      ),
                      ButtonBar(
                        alignment: MainAxisAlignment.spaceEvenly,
                        children: [
                          FloatingActionButton(
                            onPressed: () {
                              setState(() {
                                typingText += "7";
                                equalsPressed = false;
                                List<String> parts = typingText.split(
                                    RegExp(r'(?=[+\-x÷%])|(?<=[+\-x÷%])'));

                                print(parts);
                                //need at least 3 elements for an operation
                                if (parts.length >= 3) {
                                  String previous_Element =
                          ...
                          FloatingActionButton(
                            onPressed: () {
                              setState(() {
                                if (typingText.isEmpty ||
                                    typingText[typingText.length - 1] == "x") {
                                } else if (typingText[typingText.length - 1] ==
                                        "÷" ||
                                    typingText[typingText.length - 1] == "-" ||
                                    typingText[typingText.length - 1] == "+") {
...
class _calcDisplayState extends State<calcDisplay> {
  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(
          color: Color(0xff595959), borderRadius: BorderRadius.circular(10)),
